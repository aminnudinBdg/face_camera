...
class FaceIdentifier {
  ...
  static _extractFace(List<Face> faces) {
    ...
    for (Face face in faces) {
      ...
      if (face.headEulerAngleY! > eulerValue ||
          face.headEulerAngleY! < -eulerValue) {
        wellPositioned = false;
      }

      // Head is tilted sideways rotZ degrees
      if (face.headEulerAngleZ! > eulerValue ||
          face.headEulerAngleZ! < -eulerValue) {
        wellPositioned = false;
      }

      // Head is tilted sideways rotX degrees
      if (face.headEulerAngleX! > eulerValue ||
          face.headEulerAngleX! < -eulerValue) {
        wellPositioned = false;
      }
    ...
    return DetectedFace(
      ...
      face: detectedFace,
      isMultipleFaces: faces.length > 1,
    );
  }
}
